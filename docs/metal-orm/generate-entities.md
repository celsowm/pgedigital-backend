...
In this repo we wrap that command with `scripts/generate-entities.ps1`; running `npm run generate:entities` now defaults to `--out-dir=src/entities` with `--out=src/entities/index.ts`, so every table lands in its own file plus a shared barrel (override via `-Out`/`-OutDir` if you still need a single file).
