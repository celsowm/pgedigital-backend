...
In this repo we wrap that command with `scripts/generate-entities.ps1`; running `npm run generate:entities` now defaults to `--out-dir=src/entities/generated` with `--out=src/entities/generated/index.ts`, so every table lands in its own file plus a shared barrel inside `src/entities/generated` (override via `-Out`/`-OutDir` if you still need a different layout). The hand-maintained `src/entities/index.ts` imports from `./generated` and applies hooks without being overwritten.
