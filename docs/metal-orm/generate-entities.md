...
In this repo we wrap that command with `scripts/generate-entities.ps1`; running `npm run generate:entities` defaults to `--out-dir=src/entities` with `--out=src/entities/entities.generated.ts`, so every table lands in its own file directly under `src/entities` plus a shared barrel (`entities.generated.ts`). App-owned code stays separate (`src/entities/index.ts`, `src/entities/entities.ts`, and `src/entities/hooks/*`) and is not overwritten.
